import { AddressLike, BaseContract, BytesLike, Contract, ContractRunner, FunctionFragment } from "ethers";
 ...
 * 钻石合约的类型，包含多个功能接口。
 */
export type Diamond = DiamondCutFacet &
  DiamondLoupeFacet &
  OwnershipFacet & {
    /**
     * 钻石合约的地址。
...
export class Diamonds {
  ...
  static deploy = async (isVerify: boolean): Promise<Diamond> => {
    ...
    // 部署各个Diamond合约的关键组件
    // 部署各个Diamond合约的关键组件
    const [diamondInit, diamondCutFacet, diamondLoupeFacet, ownershipFacet] = await deploy(
      isVerify,
      "DiamondInit",
      "DiamondCutFacet",
      "DiamondLoupeFacet",
    ...
    // 创建Diamond合约实例
    const diamond = new Contract(diamondProxy.address, DiamondAbi, owner) as unknown as Diamond;
  ...
  static from = (diamond: string, runner?: ContractRunner | null | undefined): Diamond => {
    // 创建一个Diamond合约实例并返回
    return Diamonds.withAction(new Contract(diamond, DiamondAbi, runner) as unknown as Diamond);
  };
  ...
  static parseSelectors = (contract: BaseContract, funcHeaderOrSigs?: string[]) => {
    ...
    selectors.names = function () {
      const names = {} as { [name: string]: string };
      contract.interface.forEachFunction((func: FunctionFragment) => {
        for (const selector of this) {
...
class DiamondAction {
  ...
  private do = async (
    oldFacet: AddressLike | undefined,
    newFacet: BaseContract,
    init?: AddressLike,
    initData?: BytesLike,
  ) => {
  ...
  /**
   * 根据旧的 Facet 和新的 Facet，编码 DiamondCut 操作
   * @param oldFacet 旧的 Facet 地址或者 undefined
   * @param newFacet 新的 Facet 对象
   * @param init 初始化地址
   * @param initData 初始化数据
   * @returns 编码后的 DiamondCut 操作数据
   */
  private encodeDo = async (
    oldFacet: AddressLike | undefined,
    newFacet: BaseContract,
    init?: AddressLike,
    initData?: BytesLike,
